...
async function checkData() {
    ...
    try {
        ...
        const result = await session.run("MATCH (u:User {role: 'alumni', account_status: 'approved'}) RETURN u.id, u.name, u.email LIMIT 10");
        if (result.records.length === 0) {
            console.log('No approved alumni found.');
            console.log('\n--- Checking ALL Alumni (Any Status) ---');
            const allAlumni = await session.run("MATCH (u:User {role: 'alumni'}) RETURN u.id, u.name, u.account_status LIMIT 10");
...
checkData();
//# sourceMappingURL=check_alumni_data.js.map