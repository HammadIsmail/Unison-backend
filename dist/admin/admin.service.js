...
let AdminService = class AdminService {
    neo4j;
    mail;
    constructor(neo4j, mail) {
        this.neo4j = neo4j;
        this.mail = mail;
    }
    async getPendingAccounts() {
        const result = await this.neo4j.run(`MATCH (u:User {account_status: 'pending'})
    ...
    async approveAccount(id) {
        const result = await this.neo4j.run(`MATCH (u:User {id: $id})
       SET u.account_status = 'approved'
       RETURN u`, { id });
        if (!result.records.length) {
            throw new common_1.NotFoundException('Account not found or role mismatch.');
        }
        const user = result.records[0].get('u').properties;
        await this.mail.sendApprovalEmail(user.email, user.name);
        return { message: 'Account approved. Email sent to user.' };
    }
    async rejectAccount(id, dto) {
        const result = await this.neo4j.run(`MATCH (u:User {id: $id})
    ...
    async getAllAlumni(page, limit, search) {
        const skip = (page - 1) * limit;
        const searchCondition = search
            ? `AND toLower(u.name) CONTAINS toLower($search)`
            : '';
        const countResult = await this.neo4j.run(`MATCH (u:User {role: 'alumni', account_status: 'approved'})
       WHERE 1=1 ${searchCondition}
       RETURN count(u) AS total`, { search });
        const total = countResult.records[0]?.get('total').toNumber() || 0;
        const result = await this.neo4j.run(`MATCH (u:User {role: 'alumni', account_status: 'approved'})
       WHERE 1=1 ${searchCondition}
       ...
       SKIP toInteger($skip) LIMIT toInteger($limit)`, { search, skip, limit });
    ...
    async getAllStudents(page, limit, search) {
        const skip = (page - 1) * limit;
        const searchCondition = search
            ? `AND toLower(u.name) CONTAINS toLower($search)`
            : '';
        const countResult = await this.neo4j.run(`MATCH (u:User {role: 'student', account_status: 'approved'})
       WHERE 1=1 ${searchCondition}
       RETURN count(u) AS total`, { search });
        const total = countResult.records[0]?.get('total').toNumber() || 0;
        const result = await this.neo4j.run(`MATCH (u:User {role: 'student', account_status: 'approved'})
       WHERE 1=1 ${searchCondition}
